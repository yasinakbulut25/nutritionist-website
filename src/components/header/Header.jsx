      ...
      >
        <div className="relative max-w-7xl w-full flex items-center justify-between mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-md sm:px-6 px-2 pr-3 py-4 rounded-2xl shadow-lg">
          <div className="flex items-center sm:gap-4 gap-2">
            ...
            <Link
              href={BASE_URL}
              className="font-allura flex items-center gap-2 md:text-2xl text-xl"
            >
              <Image
                width={35}
                height={35}
                    ...
                    <AnimatePresence>
                      {hoveredDropdown === index && (
                        <motion.div
                          ...
                          className="absolute top-full left-0 mt-2 max-w-72 min-w-44 w-max bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden"
                        >
                          <div className="py-2">
                            {route.submenu.map((item, subIndex) => (
                              <Link
                                key={subIndex}
                                href={item.href}
                                className="flex flex-col px-4 py-3 hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-colors duration-150 group min-w-max"
                              >
          ...
          <div className="flex items-center gap-3">
            <ButtonWhatsapp>
              <span className="sm:flex hidden">İletişime Geç</span>
            </ButtonWhatsapp>
          </div>
        </div>
      </motion.div>
